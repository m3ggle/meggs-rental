...
const NotifyModal = () => {
  ...
  return (
    <ModalWrapper isOpen={isOpen} closeModal={closeNotifyModal}>
      <Dialog.Panel className="absolute bottom-0 top-20 flex w-full flex-col-reverse overflow-hidden rounded-t-2xl bg-white shadow-2xl 600:static 600:h-[700px] 600:w-[572px] 600:rounded-2xl 1000:h-[500px] 1000:w-[900px] 1000:flex-row">
        ...
        <div className="absolute top-0 w-full 600:relative 600:min-h-[260px] 600:w-[572px] 1000:h-[500px] 1000:w-[450px]">
          {/* gradient */}
          <div className="absolute -bottom-[1px] flex h-[36%] w-full bg-gradient-to-t from-white 1000:hidden"></div>
          <button
            onClick={closeNotifyModal}
            className="absolute right-6 top-8 flex items-center justify-center rounded-full bg-white p-3 shadow-md 600:right-12 600:top-12"
          >
            <div className="h-6 max-h-[24px] w-6 max-w-[24px]">
              <i className="fa-solid fa-times text-[24px] text-lmGrey800 drop-shadow " />
            </div>
          </button>
          {photoUrl !== null && (
            <img
